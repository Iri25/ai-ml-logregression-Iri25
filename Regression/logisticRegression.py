...
class LogisticRegression:
    ...
    def fit(self, x, y, learningRate=0.001, noEpochs=1000):
        ...
        self.coefficient_ = [0.0 for _ in range(1 + len(x[0]))]
        ...
        for epoch in range(noEpochs):
            ...
            for i in range(len(x)):
                ...
                # estimate the output
                ycomputed = sigmoid(self.evaluation(x[i], self.coefficient_))
                ...
                for j in range(0, len(x[0])):
                    self.coefficient_[j + 1] = self.coefficient_[j + 1] - learningRate * crtError * x[i][j]
                self.coefficient_[0] = self.coefficient_[0] - learningRate * crtError * 1
